...
class MainActivity : AppCompatActivity(), IBluetoothEventListener {
    ...
    private fun clickListeners() {
        ...
        btnDivide.setOnClickListener {
            sendNumbersToSocket(Actions.DIVIDE)
        }
        btnMultiply.setOnClickListener {
            sendNumbersToSocket(Actions.MULTIPLY)
        }
    }
    ...
    /*
    * Validate numbers and send to remote socket for calculation
    * */
    private fun sendNumbersToSocket(action: Actions) {
        if (et1.text!!.trim().isNotEmpty() || et2.text!!.trim().isNotEmpty()) {
            if (hasBtConnection.value != null && hasBtConnection.value!!) {
                //Send msg to second app
                val numOne = et1.text.toString().toFloat()
                val numTwo = et2.text.toString().toFloat()
                bluetoothConnection.sendMsg(Gson().toJson(CalcRequestModel(action, numOne, numTwo)))
            } else {
                connectToSecondBtDevice()
            }
        } else {
    ...
    * */
    private fun displayRemoteCalculation(dataReceived: CalcRequestModel) {
        runOnUiThread {
            tvRemoteResult.text = dataReceived.result.toString()
            etRemote1.setText(dataReceived.valueOne.toString())
            etRemote2.setText(dataReceived.valueTwo.toString())
            if (dataReceived.action == Actions.DIVIDE) {
                btnRemoteMultiply.visibility = View.INVISIBLE
                btnRemoteDivide.visibility = View.VISIBLE
            } else {
                btnRemoteDivide.visibility = View.INVISIBLE
                btnRemoteMultiply.visibility = View.VISIBLE
            }
        }
    }
